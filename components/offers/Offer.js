...
function Offer(props) {
    return (
            <Card>
                <Card.Title>
                <br/> {/*proszę nie bij za to XD */}
                <h1 className="text-center">Nazwa stanowiska</h1>
                </Card.Title>
                <Card.Body>
                    <p>City: {props.city}, {props.street}</p>
                    <p>Company: {props.company_name}</p>
                    <p>Seniority: {props.experience_level}</p>
                    <p>Salary: from <strong>{props.salary_from}</strong> to <strong>{props.salary_to}</strong></p>
                    <p>Category: {props.marker_icon}</p>
